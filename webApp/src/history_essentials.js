...
getTranslationHistory().then((historyJSON) => {
    ...
    history.forEach(element => {
        document.getElementById("transcription_history").appendChild(createView(element));
    });
});
getTranscribeHistory().then((historyJSON) => {
    const history = JSON.parse(historyJSON).history;
    history.forEach(element => {
        document.getElementById("translation_history").appendChild(createView(element));
    });
});
getDiarizationHistory().then((historyJSON) => {
    const history = JSON.parse(historyJSON).history;
    history.forEach(element => {
        document.getElementById("diarization_history").appendChild(createView(element));
    });
});